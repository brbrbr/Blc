...
class BlcPluginActor extends BlcPlugin implements SubscriberInterface, BlcExtractInterface
{
    ...
    protected function parseContainer(int $id): void
    {
        ...
        try {
            ...
            $this->loadLanguage();
            Factory::getApplication()->enqueueMessage(Text::_("PLG_BLC_SPPAGEBUILDER_QUERY_ERROR") . ' : ' . $e->getMessage(), 'error');
            return;
        }
    ...
    protected function parseContainerFields($row): void
    {
        ...
        foreach (['text', 'content'] as $field) {
            ...
            if ($this->contentFields) {
                $this->processText($this->contentFields, $field, $synchedId);
            }
            if ($this->contentLinks) {
                $this->processLinks($this->contentLinks, $field, $synchedId);
            }
        }
